...
const Search: FC = () => {
  ...
  const onNextPageHandler = async (e:any) => {
    e.preventDefault();
    fetchItems();
    setPageCounter(pageCounter + 1);
  };

  const onPreviousPageHandler = async (e:any) => {
    e.preventDefault();
    fetchItems();
    setPageCounter(pageCounter - 1);
  };

  const addToLocalStorage = (e:any, item: ResultsInterface) => {
    e.preventDefault();
    ...
    if (existing == undefined) {
      ...
      const isExist = arr.some((val:any) => {
        if (val.id === item.id) {
          return true;
        }
      });
            ...
            {results.map((result: ResultsInterface, index: number) => {
              return (
                <DisplayCard
                  ...
                  result={result}
                  isBookmark={false}
                  onAdd={(e:any) => addToLocalStorage(e, result)}
                  onDelete={undefined}
                />
              );
            })}
